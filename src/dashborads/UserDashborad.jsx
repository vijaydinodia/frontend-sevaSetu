...
const UserDashborad = () => {
  ...
  // Payment states
  const [paymentModal, setPaymentModal] = useState({ show: false, bookingId: null, method: '' })
  ...
  const handlePaymentSubmit = async () => {
    ...
    try {
      const res = await axios.put(
        `${API_URL}/user/bookings/${paymentModal.bookingId}/pay`,
        { paymentMethod: paymentModal.method },
        getHeaders()
      )
      if (res.data?.success) {
        setPaymentModal({ show: false, bookingId: null, method: '' })
        fetchBookings()
      }
    } catch (err) {
    ...
    <>
      ...
      <div className="mx-auto max-w-7xl px-5 py-8">
        ...
        <Card className="p-6 mb-8">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            ...
            <Button variant="outline" onClick={() => setShowEditProfile(true)} className="self-start sm:self-center" title="Edit Profile">
        ...
        {loadingBookings ? (
          ...
          <Card className="p-8 text-center text-[var(--text-muted)] border border-dashed border-[var(--border-color)]">
            ...
            <Button variant="gradient" onClick={() => navigate('/')} title="Browse & Book Services">
                  ...
                  <Card key={booking._id} className="p-5 border border-[var(--border-color)]">
                    <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                      <div>
                        ...
                        {(booking.status === 'pending' || booking.status === 'accepted' || booking.status === 'on_the_way') && booking.startOtp && (
                          <div className="mt-2 inline-block rounded-lg bg-indigo-50 px-3 py-1.5 border border-indigo-100">
                            <p className="text-xs text-indigo-800 m-0 font-semibold">
                              Start OTP: <span className="tracking-widest text-lg ml-1">{booking.startOtp}</span>
                            </p>
                          </div>
                        )}
                        ...
                        {booking.status === 'started' && booking.endOtp && (
                          <div className="mt-2 inline-block rounded-lg bg-indigo-50 px-3 py-1.5 border border-indigo-100">
                            <p className="text-xs text-indigo-800 m-0 font-semibold">
                              End OTP: <span className="tracking-widest text-lg ml-1">{booking.endOtp}</span>
                            </p>
                          </div>
                        )}
                      ...
                      <div className="flex flex-row md:flex-col md:items-end justify-between items-center gap-2 shrink-0 pt-4 md:pt-0 border-t md:border-0 border-[var(--border-color)]">
                        <div>
                          <p className="text-xs text-[var(--text-muted)] m-0 md:text-right">Price</p>
                          <p className="text-lg font-bold text-amber-500 m-0 text-right">₹{booking.amount}</p>
                        </div>
                        {booking.status === 'completed' && (
                          <div className="flex flex-col gap-2 w-full md:w-auto mt-2">
                            {booking.paymentStatus === 'completed' ? (
                              <div className="bg-emerald-50 dark:bg-emerald-900/20 px-3 py-2 rounded-xl border border-emerald-100 dark:border-emerald-800 md:text-right">
                                <p className="text-xs font-semibold text-emerald-600 dark:text-emerald-400 m-0">Paid via {capitalizeWords(booking.paymentMethod)}</p>
                                <p className="text-[10px] text-emerald-600/70 dark:text-emerald-400/70 m-0 mt-0.5">{new Date(booking.paymentDate).toLocaleDateString()}</p>
                              </div>
                            ) : (
                              <Button
                                size="sm"
                                variant="gradient"
                                className="w-full"
                                onClick={() => setPaymentModal({ show: true, bookingId: booking._id, method: '' })}
                              >
                                Pay Now
                              </Button>
                            )}
      ...
      {paymentModal.show && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
          <div className="bg-[var(--bg-shell)] rounded-3xl w-full max-w-sm p-8 shadow-2xl border border-[var(--border-color)]">
            <h3 className="text-xl font-bold mb-6 text-center text-[var(--text-main)]">Complete Payment</h3>
            
            <div className="mb-6">
              <label className="block text-xs font-bold uppercase tracking-wider mb-2 text-[var(--text-muted)]">Select Payment Method *</label>
              <select
                value={paymentModal.method}
                onChange={(e) => setPaymentModal(prev => ({ ...prev, method: e.target.value }))}
                className="w-full rounded-xl border border-[var(--border-color)] bg-[var(--bg-input)] px-4 py-3 text-sm text-[var(--text-main)] focus:outline-none focus:border-amber-500"
              >
            ...
            <div className="flex gap-3">
              <Button
                variant="outline"
                className="flex-1"
                onClick={() => setPaymentModal({ show: false, bookingId: null, method: '' })}
              >
                Cancel
              </Button>
              <Button
                variant="gradient"
                className="flex-1"
                disabled={actionLoading || !paymentModal.method}
