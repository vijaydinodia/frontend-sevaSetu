  ...
  return (
    <main className={`min-h-screen transition-colors duration-200 ${bgTheme}`}>
          ...
          <div className="mt-10 flex flex-wrap gap-4">
            {user ? (
              <Link to={getDashboardLink()}>
                <Button variant="gradient" className="h-14 px-7" title="Go to Dashboard">
              ...
              <Link to="/signup">
                <Button variant="gradient" className="h-14 px-7" title="Start for free">
                  <ArrowForwardOutlinedIcon fontSize="small" />
                  Start for free
                </Button>
              </Link>
            ...
            <Link to="/become-provider">
              <Button variant="outline" className="h-14 px-7" title="Join as Partner">
      ...
      <section id="how" className="mx-auto max-w-7xl px-5 py-16">
        <h2 className="text-3xl font-bold mb-2 text-center">How SevaSetu Works</h2>
        <p className={`text-center text-sm ${textMuted} mb-12 max-w-md mx-auto`}>
          Getting reliable local services is now simple, safe, and transparent. Follow these three easy steps.
        </p>

        <div className="grid gap-8 md:grid-cols-3">
          {/* Step 1 */}
          <Card className={`p-6 border text-center flex flex-col items-center gap-4 ${cardTheme}`}>
            <div className="flex h-12 w-12 items-center justify-center rounded-2xl bg-amber-500 text-white font-bold text-lg shadow-md">
              1
            </div>
            <h3 className="text-lg font-bold">Choose a Service</h3>
            <p className={`text-xs ${textMuted} leading-5`}>
              Browse through our wide variety of verified local service categories. Compare prices and partner reviews to find the perfect match.
            </p>
          </Card>

          {/* Step 2 */}
          <Card className={`p-6 border text-center flex flex-col items-center gap-4 ${cardTheme}`}>
            <div className="flex h-12 w-12 items-center justify-center rounded-2xl bg-pink-500 text-white font-bold text-lg shadow-md">
              2
            </div>
            <h3 className="text-lg font-bold">Book Instantly</h3>
            <p className={`text-xs ${textMuted} leading-5`}>
              Select a convenient time slot, enter your location details, and book securely. Our automated matching matches the best partner for you.
            </p>
          </Card>

          {/* Step 3 */}
          <Card className={`p-6 border text-center flex flex-col items-center gap-4 ${cardTheme}`}>
            <div className="flex h-12 w-12 items-center justify-center rounded-2xl bg-orange-500 text-white font-bold text-lg shadow-md">
              3
            </div>
            <h3 className="text-lg font-bold">Service Delivered</h3>
            <p className={`text-xs ${textMuted} leading-5`}>
              Track your booking status on your interactive dashboard. Our verified partner arrives at your location and gets the work done.
            </p>
          </Card>
        </div>
      </section>
      ...
      <section id="services" className={`px-5 py-16 ${theme === 'light' ? 'bg-[#faf6f0]' : 'bg-zinc-900'} border-t border-b ${theme === 'light' ? 'border-zinc-200' : 'border-zinc-800'}`}>
        <div className="mx-auto max-w-7xl">
          ...
          <div className="mb-10 max-w-2xl mx-auto">
            ...
            {/* Datamuse Suggestion Chips */}
            {suggestedKeywords.length > 0 && (
              <div className="mt-4 flex flex-wrap gap-2 items-center">
                <span className={`text-xs font-semibold ${textMuted}`}>Suggested:</span>
          ...
          {filteredServices.length === 0 ? (
            ...
            <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
              {filteredServices.map((ps) => (
                <Card
                  key={ps._id}
                  className={`flex flex-col border p-4 ${cardTheme} hover:shadow-xl transition-all duration-300 rounded-[28px] overflow-hidden group hover:-translate-y-1 hover:border-amber-500/30`}
                >
                  {ps.service?.image && (
                  ...
                  <div className="flex-1 flex flex-col justify-between">
                    <div>
                      ...
                      <div className={`mt-auto border-t pt-4 flex items-center justify-between ${theme === 'light' ? 'border-zinc-100' : 'border-zinc-800'}`}>
                        <div>
                          <p className="text-[10px] text-zinc-400 font-bold uppercase tracking-widest">Provided By</p>
                          <p className="text-sm font-semibold m-0 mt-0.5 truncate max-w-[120px]">
        ...
        <div className="mx-auto max-w-7xl">
          ...
          {categories.length === 0 ? (
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              {categories.map((cat) => (
                <Link to={`/category/${cat._id}`} key={cat._id} className="block group">
                  <Card className={`h-full p-5 text-center flex flex-col items-center gap-3 border ${cardTheme} group-hover:-translate-y-1 transition duration-200`}>
                    {cat.image ? (
                    ...
                    <h4 className="font-bold text-base m-0 group-hover:text-amber-500 transition-colors">{capitalizeWords(cat.name) || 'N/A'}</h4>
